...
const MultipleImageContent = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:5000/api/MultipleImageContentroutes/new');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = editingUserId 
        ? await axios.put(`http://127.0.0.1:5000/api/MultipleImageContentroutes/${editingUserId}`, data, {
            headers: {
              'Content-Type': 'multipart/form-data',
            },
          }) 
        : await axios.post('http://127.0.0.1:5000/api/MultipleImageContentroutes', data, {
            headers: {
              'Content-Type': 'multipart/form-data',
            },
          });
